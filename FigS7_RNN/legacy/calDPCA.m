% the helper code to calculate dpca loadings
clear all; clc
...
folder = dir('~/Documents/MATLAB/ChandLab/RNN_data/new2/4AreasA6*.mat');
...
%     for kk = 1:size(fr_raw,1)
%         temp = squeeze(fr_raw(kk,:,:,:));
%         temp = (temp - min(temp(:)))./(max(temp(:)) - min(temp(:)) + 0.001);
%         fr(kk,:,:,:) = temp;
%     end
...
%      for kk = 1:size(fr_raw,1)
%         temp = squeeze(fr_raw(kk,:,:,:));
%         temp = (temp - mean(temp(:)))./(std(temp(:)) + 0.0001);
%         fr(kk,:,:,:) = temp;
%     end   
