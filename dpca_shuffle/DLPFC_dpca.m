...
% aligned to checkerboard (-0.8:0.8)
% firingRatesAverage = load('/Volumes/TianSSD/TiberiusDLPFCforDPCA/checkerboardAligned/totalDataframeC.mat').totalDataframe;

% aligned to target (-0.4:1.2)
% firingRatesAverage = load('/Volumes/TianSSD/TiberiusDLPFCforDPCA/targetAligned/totalDataframeT.mat').totalDataframe;

% time restretching aligned to target (-0.4:2)
firingRatesAverage = load('/Volumes/TianSSD/TiberiusDLPFCforDPCA/all_new/totalDataframeA.mat').totalDataframe;
% firingRatesAverage = load('/Volumes/TianSSD/TiberiusDLPFCforDPCA/all_SC_Dir/totalDataframeA.mat').totalDataframe;
% firingRatesAverage = load('/Volumes/TianSSD/TiberiusDLPFCforDPCA/all_SC_Context/totalDataframeAB5.mat').totalDataframe;
% firingRatesAverage = load('/Volumes/TianSSD/TiberiusDLPFCforDPCA/all_new/totalDataframeAB1_4.mat').totalDataframe;

% aligned to target; plot signed coherence and target configuration 
% firingRatesAverage = load('/Volumes/TianSSD/TiberiusDLPFCforDPCA/targetAligned_SC_Context/totalDataframeTSC.mat').totalDataframe;

% aligned to checkerboard: plot signed coherence and target configuration 
% firingRatesAverage = load('/Volumes/TianSSD/TiberiusDLPFCforDPCA/checkerboardAligned_SC_Context/totalDataframeCSC.mat').totalDataframe;


% firingRatesAverage = firingRatesAverage(:,:,:,301:1600);
% firingRatesAverage = firingRatesAverage(1598:end,:,:,1:1300);
...
margNames = {'SC', 'Condition-independent'};
...
time = linspace(-0.8, 0.5, size(firingRatesAverage, 4));
timeEvents = 0;

% time of  target aligned
% time = linspace(-0.4, 1.2, size(firingRatesAverage, 4));
% timeEvents = [0];
...
% print('-painters','-depsc',['~/Desktop/', 'dpca_AA','.eps'], '-r300');
