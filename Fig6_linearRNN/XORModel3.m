classdef XORModel3 < handle
    ...
    properties
        ...
        % Model type
        model_type              % 'switching', 'flipping_input', 'rotating_input_symmetric'
        rotation_angle = 180    % Rotation angle in degrees (for rotating_input_symmetric)
        ...
        J                       % Fixed J (for rotating models)
        
        % Input structure
        B_context               % Context input weights
        B_color                 % Color input weights
        B_choice                % Choice input weights
        B_ci                    % Condition-independent input
        
        I0_context = 0.7        % Context input strength
        I0_color = 0.6          % Color input strength
    ...
    methods
        function obj = XORModel3(model_type, rotation_angle)
            ...
            if strcmp(model_type, 'rotating_input_symmetric')
        ...
        function obj = build_network(obj)
            ...
            obj.Lambda_base(1,1) = 0.2;  % Context - weak
            obj.Lambda_base(2,2) = 0.15;   % Color - transient
            obj.Lambda_base(3,3) = 0.9;  % Choice - stable
            ...
            switch obj.model_type
                ...
                case {'flipping_input', 'rotating_input_symmetric'}
                    % One fixed J matrix with coupling
                    J_coupling = obj.coupling_strength * (choice_dir * color_dir');
                    obj.J = obj.J_base + J_coupling;
                    fprintf('  Built fixed J with coupling (rotating/flipping input)\n');
            end
        end
        
        function obj = build_inputs(obj)
            % Build input structure with heterogeneous tuning
            
            % Heterogeneous tuning for all input dimensions
            ...
            choice_tuning = abs(randn(obj.N, 1));
            choice_tuning = choice_tuning / mean(choice_tuning);
            ...
            obj.B_choice = obj.Q(:,3) .* choice_tuning;
        ...
        function obj = simulate(obj)
            ...
            for cond = 1:4
                ...
                % For flipping/rotating input models, modify the color sign based on rotation
                if strcmp(obj.model_type, 'flipping_input')
                    % Perfect 180° flip: reverse sign for T2
                    if context_sign == -1
        ...
        function input = compute_input(obj, x_t, context_sign, color_sign_rotated, current_time)
            ...
            if current_time >= obj.T_color_onset + 30
                ...
                if strcmp(obj.model_type, 'rotating_input_symmetric')
                    % Rotated input: add both color and choice components with heterogeneous tuning
                    input = input + obj.rotated_color_component * obj.B_color * obj.I0_color;
                    input = input + obj.rotated_choice_component * obj.B_choice * obj.I0_color;
                    
                else
                    % All other models: standard color input (switching, flipping_input)
