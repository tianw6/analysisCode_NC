function [h, area, angle]=error_ellipse(varargin)
...
%
%    OUTPUTS:
%      h - graphics handle(s) [empty since plotting removed]
%      area - area of the error ellipse
%      angle - angle of the ellipse in degrees [-90, 90] (2D only, NaN for 3D)
%
...
% calculate the area of error ellipse
area=prod(sqrt(eig(C)))*pi*k;
...
%---------------------------------------------------------------
% compute_angle - Compute the angle of the ellipse
function angle = compute_angle(C)

[eigvec,eigval] = eig(C); % Compute eigen-stuff
[~,i] = sort(diag(eigval));

% angle of the ellipse
angle = rad2deg(atan(eigvec(2,i(2))./eigvec(1,i(2))));
disp(['Angle of the ellipse: ', num2str(angle)]);
