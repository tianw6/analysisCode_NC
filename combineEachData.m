...
for dayn = 1:length(folder)
    ...
    rFR_RL = mean(squeeze(FRmatrix(:,:,RL)),3);
    rFR_RR = mean(squeeze(FRmatrix(:,:,RR)),3);
    rFR_GL = mean(squeeze(FRmatrix(:,:,GL)),3);
    rFR_GR = mean(squeeze(FRmatrix(:,:,GR)),3);

    FRaverage(:,1,1,:) = rFR_RL;
    FRaverage(:,1,2,:) = rFR_RR;
    FRaverage(:,2,1,:) = rFR_GL;
    FRaverage(:,2,2,:) = rFR_GR;
