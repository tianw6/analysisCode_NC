classdef XORModel < handle
    ...
    properties
        ...
        B_color                 % Color input weights (or B_color_base for rotating)
        B_color_base            % Base color weights (rotating model)
    ...
    methods
        function obj = XORModel(model_type)
            % Constructor
            if nargin < 1
        ...
        function obj = build_network(obj)
            ...
            obj.Lambda_base(2,2) = 0.2;   % Color - transient
            obj.Lambda_base(3,3) = 0.95;  % Choice - stable
            obj.Lambda_base(4,4) = 0.9;   % CI - stable
            for i = 5:obj.N
        ...
        function obj = build_inputs(obj)
            ...
            if strcmp(obj.model_type, 'rotating_input')
                % Store base color weights (will be flipped by context)
                obj.B_color_base = obj.Q(:,2) .* color_tuning;
            else
                % Fixed color weights
        ...
        function obj = simulate(obj)
            ...
            for cond = 1:4
                ...
                % Select color input based on model type
                B_color_active = obj.select_color_input(context_sign);
                ...
                for t = 1:(n_time-1)
                    ...
                    % Compute input
                    input = obj.compute_input(x(:,t), context_sign, color_sign, ...
                                             current_time, B_color_active);
        ...
        function J = select_dynamics(obj, context_sign)
            ...
            switch obj.model_type
                ...
                case {'rotating_input', 'gain_modulation_input', 'gain_modulation_state'}
                    J = obj.J;
            end
        end
        
        function B_color = select_color_input(obj, context_sign)
            % Select color input direction based on model
            
            switch obj.model_type
                case 'rotating_input'
                    % Rotate input direction with context
                    if context_sign == 1
                        B_color = obj.B_color_base;
                    else
                        B_color = -obj.B_color_base;  % FLIP!
                    end
                case {'switching', 'gain_modulation_input', 'gain_modulation_state'}
                    % Fixed color input
                    B_color = obj.B_color;
            end
        end
        
        function input = compute_input(obj, x_t, context_sign, color_sign, ...
                                       current_time, B_color_active)
            ...
            if current_time >= obj.T_color_onset + 30
                ...
                if strcmp(obj.model_type, 'gain_modulation_input')
                    ...
                    % NO direct color input - color only matters via gain modulation
                    % Predicts: Minimal/zero color representation (immediate conversion to choice)
                    context_state = obj.Q(:,1)' * x_t;
                    gain = context_state;
                    color_input_strength = color_sign * obj.I0_color;
                ...
                elseif strcmp(obj.model_type, 'gain_modulation_state')
                    ...
                    % Needs direct color input to build up color state first
                    % Predicts: Moderate color representation (needs to accumulate first)
                    input = input + color_sign * B_color_active * obj.I0_color;
                    ...
                    % All other models: standard color input
                    input = input + color_sign * B_color_active * obj.I0_color;
                end
            end
        end
        ...
        function dPCA(obj)
            ...
            % firingRatesAverage(neuron, stimulus, decision, time)
            % stimulus: 1=Red, 2=Green
            % decision: 1=Left, 2=Right
