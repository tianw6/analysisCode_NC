...
Lambda_base(1,1) = 0.4;  % Context - weaker (more decay)
Lambda_base(2,2) = 0.4;  % Color - even weaker (transient)
Lambda_base(3,3) = 0.7;  % Choice (stable, strong)
for i = 4:N
    Lambda_base(i,i) = 0.1 + 0.1*rand();
end
...
% ACTIVE SUPPRESSION: Choice suppresses context and color
J_nonnormal(1,3) = -nonnormal_strength * 0.8;  % Choice → -Context (strong suppression)
J_nonnormal(2,3) = J_nonnormal(2,3) - nonnormal_strength * 0.6;  % Choice → -Color (moderate suppression)

% Transform to neural space
J_nonnormal_full = Q * J_nonnormal * Q';
J_base = J_base + J_nonnormal_full;

fprintf('Added nonnormal dynamics with choice suppression of context/color\n');

%% Build two different systems with different color→choice coupling

coupling_strength = .5;  % Strong coupling amplifies choice
...
I0_context = .5;  % Weak context input
I0_color = 1;    % Weak color input
...
xlabel('PC1', 'FontSize', 13, 'FontWeight', 'bold');
ylabel('PC2', 'FontSize', 13, 'FontWeight', 'bold');
zlabel('PC3', 'FontSize', 13, 'FontWeight', 'bold');
title('3D Neural Trajectories', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
view(45, 30);  % Nice viewing angle
legend(labels, 'Location', 'best');

% Add second view
figure('Position', [920, 100, 800, 700]);
for c = 1:4
    plot3(pc1(:,c), pc2(:,c), pc3(:,c), styles{c}, ...
        'Color', cols(c,:), 'LineWidth', 2.5);
    hold on;
...
ylabel('PC2', 'FontSize', 13, 'FontWeight', 'bold');
zlabel('PC3', 'FontSize', 13, 'FontWeight', 'bold');
title('3D Neural Trajectories (Top View)', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
