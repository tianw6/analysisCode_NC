...
[p, tbl, stats] = anovan(unit31, {red_cat, left_cat}, 'model', 'full', 'varnames', {'Red', 'Left'}, 'display', 'on');
...
% Combine all vectors into one data vector
allData = [frRL; frRR; frGL; frGR];

% Create a grouping vector
groupSize1 = length(frRL);
groupSize2 = length(frRR);
groupSize3 = length(frGL);
groupSize4 = length(frGR);

groups = [ones(groupSize1, 1); 2*ones(groupSize2, 1); 3*ones(groupSize3, 1); 4*ones(groupSize4, 1)];
groups = categorical(groups, 1:4, {'frRL', 'frRR', 'frGL', 'frGR'});

% Step 2: Perform one-way ANOVA
fprintf('Performing one-way ANOVA...\n');
[p, tbl, stats] = anova1(allData, groups, 'off');
...
if p < 0.05
    fprintf('The ANOVA result is significant (p < 0.05).\n');
    fprintf('We can proceed with post-hoc tests to determine which pairs differ.\n\n');
else
    fprintf('The ANOVA result is not significant (p >= 0.05).\n');
    fprintf('There are no significant differences between the group means.\n\n');
end


% Step 3: Perform Tukey's HSD test for multiple comparisons
fprintf('Performing Tukey''s HSD post-hoc test...\n');
[c, m, h, gnames] = multcompare(stats, 'display', 'off');

% Create a table with the results
comparison = array2table(c, 'VariableNames', {'Group1', 'Group2', 'LowerCI', 'MeanDiff', 'UpperCI', 'PValue'});
comparison.Group1 = gnames(c(:,1));
comparison.Group2 = gnames(c(:,2));
