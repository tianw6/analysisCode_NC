...
firingRatesAverage = [b(1:end,:,:); f; h];
...
firingRatesAverage = firingRatesAverage(:,1:10,:);

figure; hold on

meanFR = squeeze(mean(firingRatesAverage, 1));
plot(t(1:10), meanFR(:,1),'r')
plot(t(1:10), meanFR(:,2),'r--')
plot(t(1:10), meanFR(:,3),'g')
plot(t(1:10), meanFR(:,4),'g--')
