...
%         print('-painters','-depsc',['~/Documents/chandlab/paperFigures/Fig4/geometry/', num2str(jj), 'pfcStimulus' num2str(time(tV)), '.eps']);
...
c2 = summer(6);
 ...
 for ii = 1:6
    ...
    for jj = [1 3 5]
        ...
        plot(time, squeeze(orthF(ii, :, jj)), '-', 'color', c1(jj,:))
        plot(time, squeeze(orthF(ii, :, jj+1)), '--', 'color', c1(jj,:))

        plot(time, squeeze(orthF(ii, :, m - jj)), '-', 'color', c2(jj,:))
        plot(time, squeeze(orthF(ii, :, m-jj+1)), '--', 'color', c2(jj,:))
